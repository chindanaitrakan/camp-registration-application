...
/**
 * View the camp data which is accessible to a camp committee.
 * @author Trakantannarong Chindanai
 * @version 1.0
 * @since 2023-11-24
 */
public class CampViewerCommittee implements DataViewer{
	/**
	...
	@Override
	/**
	 * View camp data for camp committee.
	...
	public void viewData(User user, CampList campList) {
		...
		//filter the registration
		campFilter = new CampCommitteeIdFilter();
		filteredCampList = campFilter.filter(user, campList);
		...
		for(int i=0; i<filteredCampList.size(); i++) {
			...
			System.out.println("[" + count + "] "+filteredCampList.get(i).getName() 
					...
					+ "\nTotal Slots Remaining (Committee) | (Committee Slots): " + remainingSlots + " | " + remainingCommSlots 
